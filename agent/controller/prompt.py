...
PROMPT_BEFORE_MAX_STEPS = dedent('''
...
ブラウザを使用して Web を閲覧する際は、以下のルールに厳密に従ってください。\n
    ...
    - フィルターのポップオーバー（ドロップダウン・パネル）が途中で表示される可能性がある要素を操作る際は、文字入力やクリックと「送信」「決定」「検索」などを同時に生成せず、まず文字入力やクリックのみを行い、ページの状況を判断してから次のステップで「送信」「決定」「検索」などを生成してください。\n
