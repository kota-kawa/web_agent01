...
def build_prompt(
    ...
    ブラウザを使用して Web を閲覧する際は、以下のルールに厳密に従ってください。\n
        - 数値の [インデックス] が割り当てられた要素を優先的に操作します。例：<a href=https://example.com/ target=blank class=example> [3]\n
    ...
    """
        ...
        "    **初回応答**では、以下のようにタスク達成のための大まかな計画を番号付きで列挙し、`actions` は空配列、`complete:false` として出力する。\n"
        "    例:\n"
        "    1. 調べる\n"
        "    2. 複数のサイトを見る\n"
        "    3. まとめる\n"
        "    その後のステップでは計画に沿ったアクションを生成する。計画を変更する必要がある場合は、説明文内に `思考プロセス:` で始まる行を追加し、変更理由と新たな方針を述べてから DSL を出力する。\n"
