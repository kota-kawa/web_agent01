...
def build_prompt(cmd: str, page: str, hist, screenshot: bool = False) -> str:
    ...
    system_prompt = (
        ...
        ** **\n\n
        ...
        "2. `click` はCSSセレクタで指定します。**非表示要素(`aria-hidden='true'`など)を避け、ユニークな属性(id, name, data-testidなど)を優先してください。**\n"
        "3. `click_text` は可視テキストで指定します。\n"
        "4. 失敗しやすい操作には `wait` を挿入し、安定化を図ること。\n"
        "5. 類似要素が複数ある場合は `:nth-of-type()` や `:has-text()` などで特定性を高める。\n"
        "6. 一度に大量の操作を出さず、状況確認が必要な場合は `complete:false` とし段階的に進める。\n"
        "7. 一度に有効な複数の操作を出す場合には、各アクションの間に１秒の待機を設ける\n"
        "8. **ユーザーがページ内テキストを要求している場合**:\n"
        "   - `Maps` や `click` を行わずとも情報が取れるなら `actions` は空。\n"
        "   - 説明部にページから抽出したテキストを含める（長文は冒頭 200 文字＋\"...\"）。\n"
        f"9. 最大 {MAX_STEPS} ステップ以内にタスクを完了できない場合は `complete:true` で終了してください。\n"
        "\n"
        "---- 現在のページ HTML(一部) ----\n"
        f"{strip_html(page)}\n"
        "--------------------------------\n"
